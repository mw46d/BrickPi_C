/*
 * g++ -o goal '-D__LIBPIXY_VERSION__="0.2"' -I../../Drivers -I/usr/include/libusb-1.0 goal.cpp -L/usr/local/lib -Wl,-R/usr/local/lib -lpixyusb -lrt -lm -lwiringPi -lboost_thread
 */
#include <fcntl.h>
#include <math.h>
#include <memory.h>
  ...
  while (1) {
    ...
    if (blocks_copied <= 0) {
      ...
      for (int i = 0; i < blocks_copied; i++) {
        ...
        else if (searchBlock == 1) {
          if (blocks[i].type == TYPE_COLOR_CODE) {
            if (blocks[i].signature == 023) {
              unsigned int a;
...
int main(int argc, char *argv[]) {
  ...
  XColor yellow_col;
  GC yellow_gc = XCreateGC(dis, win, 0, 0);
  XParseColor(dis, colormap, yellow, &yellow_col);
  XAllocColor(dis, colormap, &yellow_col);
  ...
  while (startTime + 120 > time(NULL)) {
    ...
    while (startTime + 120 > time(NULL) && sensorTarget == 255 && !grab) {
      ...
      if (!foundBlock) {
        if (search_count++ > 3) {
          XFlush(dis);
          printf("%s: search\n", getRelativeTime(buf, 256));
          motorRunTachometer(motor1, 0, searchSpeed, 60, 1, 0, 1);
          motorRunTachometer(motor2, 0, -searchSpeed, 60, 1, 1, 1);
        }
	...
	blockMtx.lock();
        ...
        if (y < 120) {
          if (x < 140) {
            printf("%s: turn left\n", getRelativeTime(buf, 256));
            motorRunTachometer(motor1, 0, searchSpeed, 30, 1, 0, 1);
            motorRunTachometer(motor2, 0, -searchSpeed, 30, 1, 1, 1);
          }
          else if(x > 180) {
            printf("%s: turn right\n", getRelativeTime(buf, 256));
            motorRunTachometer(motor1, 0, -searchSpeed, 30, 1, 0, 1);
            ...
            printf("%s: forward\n", getRelativeTime(buf, 256));
            motorRunTachometer(motor1 | motor2, 0, searchSpeed, 240 - y, 1, 1, 1);
          }
        }
        else {
          printf("%s: grab\n", getRelativeTime(buf, 256));
	...
	blockMtx.lock();
        ...
        x_x = blck.x - blck.width / 2;
        x_y = blck.y - blck.height / 2;
        XSetForeground(dis, yellow_gc, yellow_col.pixel);
        XFillRectangle(dis, win, yellow_gc, x_x, x_y, blck.width, blck.height);
        XFlush(dis);
